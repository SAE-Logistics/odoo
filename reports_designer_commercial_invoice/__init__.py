...
def pre_init_check(env):
    ...
    if int(''.join([str(100+int(d)) for d in  reports_designer.installed_version.split('.')[2:]])) < 101104116:
        raise ValidationError('Module support "Report Designer (XLSX, XLSM)" module starting from Version 1.4.16, found Version {}. Please update the module "Report Designer (XLSX, XLSM)".'.format('.'.join(reports_designer.installed_version.split('.')[2:])))
    return True